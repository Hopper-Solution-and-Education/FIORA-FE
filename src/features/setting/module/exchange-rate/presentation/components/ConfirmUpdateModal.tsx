...
import { ExchangeRateObjectType } from '../types';

type ConfirmUpdateModalProps = {
  open: boolean;
  onUpdate: () => void;
  onClose: () => void;
  data: ExchangeRateObjectType | undefined;
  isUpdating: boolean;
};

const ConfirmUpdateModal = (props: ConfirmUpdateModalProps) => {
  const { data, onClose, onUpdate, open, isUpdating } = props;
        ...
        <div className={cn('px-4 sm:px-6 md:px-8 pt-6 sm:pt-8 pb-4 sm:pb-6', 'flex flex-col')}>
          <div className="flex flex-col items-center mb-4 sm:mb-6">
            ...
            >
              {data && (
                <>
                  You are going to update exchange rate from{' '}
                  <strong className="text-blue-600 dark:text-blue-400">
                    {data.fromValue.toLocaleString()} {data.fromCurrency} ({data.fromSymbol})
                  </strong>{' '}
                  to{' '}
                  <strong className="text-green-600 dark:text-green-400">
                    {data.toValue.toLocaleString()} {data.toCurrency} ({data.toSymbol})
                  </strong>
                </>
              )}
            </p>
            ...
            >
              Or click <span className="text-green-500">✓</span> to confirm update.
            </p>
          </div>
