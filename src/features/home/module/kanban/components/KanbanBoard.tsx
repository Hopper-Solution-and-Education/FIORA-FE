...
import { setColumns, setTasks } from '../slices';
...
export function KanbanBoard() {
  ...
  const { columns, tasks, draggedTask } = useAppSelector((state) => state.task);
  ...
  function onDragOver(event: DragOverEvent) {
    ...
    if (isActiveATask && isOverATask) {
      ...
      if (activeTask && overTask && activeTask.status !== overTask.status) {
        ...
        dispatch(setTasks(arrayMove(tasks, activeIndex, overIndex - 1)));
      }
