...
const FromSelectField: React.FC<FromSelectProps> = ({
  ...
  const options = useMemo((): DropdownOption[] => {
    ...
    if (isLoading || !supportingData || lastFetchedType !== transactionType) {
      return [
        {
          label: transactionType === 'Income' ? 'Loading categories...' : 'Loading accounts...',
          value: 'loading',
          disabled: true,
        },
      ];
    ...
    // Determine source data based on transaction type
    let sourceData: Array<{ id: string; name: string; type?: string }> = [];

    if (transactionType === 'Income') {
      // For Income: From should be Categories (income categories)
      sourceData = supportingData.fromCategories || [];
    } else if (transactionType === 'Expense' || transactionType === 'Transfer') {
      // For Expense/Transfer: From should be Accounts
      sourceData = supportingData.fromAccounts || [];
    }
    ...
    return [
      {
        label: transactionType === 'Income' ? 'No categories available' : 'No accounts available',
        value: 'none',
        disabled: true,
      },
    ];
