...
export function MonthPicker({
  ...
  const MonthView = () => {
    ...
    const currentYear = selectedMonth ? selectedMonth.getFullYear() : new Date().getFullYear();
...
export function QuarterPicker({
  ...
  const QuarterView = () => {
    const currentYear = selectedQuarter ? selectedQuarter.getFullYear() : new Date().getFullYear();
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        ...
        >
          <CalendarIcon className="mr-2 h-4 w-4" />
          {selectedQuarter ? getQuarterLabel(selectedQuarter) : <span>{placeholder}</span>}
        </Button>
      </PopoverTrigger>
      <PopoverContent className="w-auto p-0" align="start">
        <QuarterView />
...
export function YearPicker({
  ...
  const YearView = () => {
    const currentYear = selectedYear ? selectedYear.getFullYear() : new Date().getFullYear();
    ...
    return (
      <div className="p-3">
        <div className="flex justify-between items-center mb-4">
          <Button variant="outline" size="icon" onClick={() => setDecade((d) => d - 10)}>
            <ChevronLeft className="h-4 w-4" />
          </Button>
        ...
        <div className="grid grid-cols-4 gap-2">
          {years.map((year) => {
            const isCurrentYear = selectedYear && selectedYear.getFullYear() === year;
            ...
            return (
              <Button
                ...
                className={cn('h-9', !isCurrentDecade && 'text-muted-foreground opacity-50')}
                onClick={() => {
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        ...
        >
          <CalendarIcon className="mr-2 h-4 w-4" />
          {selectedYear ? format(selectedYear, 'yyyy') : <span>{placeholder}</span>}
        </Button>
      </PopoverTrigger>
      <PopoverContent className="w-auto p-0" align="start">
        <YearView />
