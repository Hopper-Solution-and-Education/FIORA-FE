...
function Nav({
  ...
  const getAriaLabel = (direction: 'previous' | 'next') => {
    ...
    return direction === 'previous' ? labelPrevious(previousMonth) : labelNext(nextMonth);
  };
...
function YearGrid({
  ...
  const startYear = displayYears.from;
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  if (selected && typeof selected === 'object') {
                    if (
                      'getMonth' in selected &&
                      typeof (selected as Date).getMonth === 'function'
                    ) {
  ...
  return (
    <div className={cn('grid grid-cols-3 gap-2', className)} {...props}>
      {monthNames.map((month, index) => {
        ...
        const isBefore =
          startMonth &&
          differenceInCalendarDays(new Date(displayYears.from, index, 1), startMonth) < 0;

        const isAfter =
          endMonth && differenceInCalendarDays(new Date(displayYears.from, index, 1), endMonth) > 0;
