...
// Schema for updating identification document (includes id field)
export const updateIdentificationDocumentSchema = Joi.object({
  ekycId: Joi.string().uuid().required().messages({
    'string.guid': 'eKYC ID must be a valid UUID',
    'any.required': 'eKYC ID is required for update',
  }),
  type: Joi.string()
    .valid(...Object.values(IdentificationType))
    .required()
