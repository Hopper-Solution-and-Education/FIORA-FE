...
import { transferSavingWalletBodySchema } from '@/shared/validators/savingWalletValidator';
...
async function POST(req: NextApiRequest, res: NextApiResponse, userId: string) {
  try {
    const { error } = validateBody(transferSavingWalletBodySchema, req.body);
    if (error) {
      return res
        .status(RESPONSE_CODE.BAD_REQUEST)
        .json(createErrorResponse(RESPONSE_CODE.BAD_REQUEST, Messages.VALIDATION_ERROR, error));
    }
    const { packageFXId, action } = req.body;

    const result = await walletUseCase.transferSavingWallet(userId, packageFXId, action);
